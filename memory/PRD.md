...
- Admin-Tab „Medien": Share-Bild, Favicon UND Logo (500×500 empfohlen, PNG/SVG/WebP/JPG ≤512 KB) hochladen/ersetzen/löschen; Persistenz in backend/uploads; öffentliche Endpunkte /api/media/{share-image|favicon|logo} mit Fallback (og-default.jpg / favicon.svg); Logo-Komponente zeigt hochgeladenes Bild (auto-skaliert h-10/h-11) oder sauberen CLEANORA-Schriftzug (Slash/X entfernt); SiteContext lädt /api/media/info.
...
- Verifikations-Runde (Commit e23dd63, siehe memory/CHANGELOG.md): Kritischer Login-Lockout-Bug (500 statt 429) behoben, X-Forwarded-For Rate-Limit, Admin-Logo-Vorschauen mit Cache-Buster/Live-Refresh/Platzhaltern, Desktop-Abstände kompakter (lg:py-28, Hero items-start). 28/28 Backend-Tests + alle Playwright-Flows grün; Regressionssuite backend/tests/test_media_logo.py.
